...
BlurHorizontalShader::BlurHorizontalShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"blurHorizontal_vs.cso", L"blurHorizontal_ps.cso");
}


BlurHorizontalShader::~BlurHorizontalShader()
{
	if (sampleState)
	{
		sampleState->Release();
...
void BlurHorizontalShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void BlurHorizontalShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float width)
{
